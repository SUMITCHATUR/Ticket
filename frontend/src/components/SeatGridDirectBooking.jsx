...
const SeatGridDirectBooking = ({ seats, selectedSeat, onSeatSelect, loading, routeId }) => {
  const handleSeatSelect = (seat) => {
    console.log('Seat clicked for direct booking:', {
      id: seat.id,
      number: seat.number,
      status: seat.status,
      type: seat.type
    ...
    // Direct booking - no restrictions, allow any seat to be selected
    onSeatSelect(seat)
  ...
  // Get seat styling based on selection only
              ...
              <div className="grid grid-cols-4 gap-2">
                {seatRows[rowLetter]?.map((seat) => (
                  <button
                    ...
                    title={`Seat ${seat.number} - ${seat.type || 'Standard'} - Click to select`}
                  >
                    {selectedSeat?.id === seat.id ? '✓' : seat.number}
                  </button>
                ))}
              </div>
            </div>
...
export default SeatGridDirectBooking
